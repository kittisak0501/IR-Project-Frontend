<template>
  <div class="filter" :class="isOpen ? 'dropdown-open' : ''" ref="dropdown">
    ...
    <a
      href="#"
      v-if="renderAsLink"
      class="dropdown-menu-toggle-link"
      @click="isOpen = !isOpen"
    >
      {{ text }}
      <i class="fa fa-angle-down" aria-hidden="true"></i>
    ...
    <div class="dropdown-menu">
      ...
      <div class="dropdown-content">
        <ul class="dropdown-header-items" v-if="header.length">
          <li v-for="item in header" :key="item">
            <a href="#" @click.prevent="handleHeaderClick(item, $event)">{{
              item.text
            }}</a>
          </li>
        </ul>
        <ul class="dropdown-items">
          <li v-for="item in items" :key="item">
            <a href="#" @click.prevent="handleItemClick(item, $event)">{{
              item.text
            }}</a>
          </li>
          <!-- Bottom message -->
          <li
            ...
            v-show="bottomMessage"
            v-html="bottomMessage"
          ></li>
        </ul>
      </div>
      ...
      <div class="dropdown-footer" v-if="footer.length">
        <ul>
          <li v-for="item in footer" :key="item">
            <a href="#" @click.prevent="handleFooterClick(item, $event)">{{
              item.text
            }}</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handleDropdownScroll() {
      this.$refs.dropdown.querySelector('.dropdown-content').addEventListener(
        'scroll',
        debounce((e) => {
    ...
    handleHeaderClick(item, e) {
      ...
      if (this.multiple) {
        ...
        let oldSelection = this.$refs.dropdown.querySelectorAll(
          '.filter li a.is-selected'
        )
      ...
      let oldSelection = this.$refs.dropdown.querySelector(
        '.filter li a.is-selected'
      )
      if (oldSelection) {
      ...
      if (this.multiple) {
        if (e.target.classList.contains('is-selected')) {
          ...
          if (typeof this.selectedValue !== 'object') {
            this.$refs.dropdown
              .querySelector('.filter li a.is-selected')
